...
4. 使用继承时尽量遵循里氏替换原则
### 4.单一职责原则
    单一职责原则规定一个类应该有且仅有一个引起它变好的原因，否则类应该被拆分。
如果一个对象承担太多的职责，至少存在两个缺点：
* 一个职责变化可能会削弱或者抑制这个类实现其他职责的能力
* 当客户端需要该对象的某一个职责时，不得不将其他不需要的职责全都包含进来，从而造成冗余代码或代码浪费
#### 4.1.单一职责原则的优点
* 降低类的复杂度，一个类只负责一项职责会比多项职责简单
* 提高类的可读性
* 提高系统的可维护性
